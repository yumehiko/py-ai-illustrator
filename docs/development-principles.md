...
Conversion CoreとStructured Authoringの公開API / CLIを使い、自然言語の依頼を安全な操作計画へ変換します。
...
skillやpluginへファイル解析・変換ロジックを重複実装しません。エージェントは低レベルbyteを直接書き換えず、typed operationまたはauthoring componentを利用します。
...
8. **層の依存方向を守る**
   - Conversion Coreはauthoringやagentへ依存せず、agent固有処理をコアへ混ぜない。
