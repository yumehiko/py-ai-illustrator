...
| --- | --- | --- | --- |
| Conversion Core | Trusted legacy conversion + modern semantic read profile v2成立 | legacy AI7/AI8の安全往復・patchに加え、modern PrivateDataのexact-span CST、group / compound / clipping、CMYK/RGB、Bézier/closepath投影、証拠付きpartial text保持 | PDF fallback、不一致診断、writer、Layer 2/3 |
...
2026-08-18時点のローカル基準は、`pytest` 204件成功、`ruff check`成功です。
...
- **modern AI read-only semantic v2**: bounded PDF / PieceInfo参照解決、PrivateData全segment、Flate / zstd展開、lossless token/section index、exact-span lexer/CSTに加え、layer / group / compound / clipping、CMYK/RGB、Bézier/closepathを`Document`へ投影。AI11 textはsource-local配置証拠が揃う場合だけ`TextFrame`化し、実機の未証明配置は本文・identity・親stack位置付きpartialとして保持
...
- [x] Illustrator 30.7.0実機fixtureのlayer / group / path / CMYK/RGB paintを共通グラフィックIRへ投影
- [x] AI11 textのstory本文とidentityを読み、未証明の配置を推測せずpartial nodeとして保持
- [x] AI11 textとsemantic hierarchyの入れ子上限、未終端path/containerの境界退避、全階層semantic node ID一意化を回帰テスト
- [x] source改名・ID重複後のpartial親参照と`q/Q` graphics state復元を回帰テスト
- [x] recursive coverage、style/container/operator exact span、unknown statement、semantic diagnosticsを公開
- [x] Illustrator 30.7.0実機fixtureでbanner group、CMYK Bézier、compound / clipping混在stackを固定
...
4. C1のAPI/CLI境界を固定できた後にC2のCodex skill/pluginへ進む
