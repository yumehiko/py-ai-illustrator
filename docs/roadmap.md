...
| --- | --- | --- | --- |
| Conversion Core | 限定subsetで実証済み | legacy AI7/AI8 subsetのIR/JSON往復、source付きreader result、parse coverage、fill / stroke / text / path translate局所patch、Illustrator 30.7.0適合試験 | 残りのnode source span / typed patch、modern AI semantic reader/writer |
...
2026-08-17時点のローカル基準は、`pytest` 125件成功、`ruff check`成功です。
...
- [ ] 全IR nodeへorigin/source spanを接続する（`Path`のfill / stroke / geometryと単一`Tx`の`TextFrame`は完了）
- [ ] text / image / pathのtyped editを局所source patchへ変換する（fill / stroke / text / path translateは完了）
...
- [ ] node-level CST/source span（`Path`のfill / stroke / geometryと単一`Tx`の`TextFrame`本文は実装済み）
- [x] source付きreader resultと新規作成IRの明確な区別
- [x] strict-by-default reserializeと明示的な`discard` loss policy
- [ ] typed patch writer（fill / stroke / text / path translateは実装済み）
...
1. Gate A / A1のsource spanをimageと複数`Tx` textへ拡張する
2. typed patchをimage差し替えとcontainer translateへ拡張する
