...
def _build_modern_roundtrip_javascript(source: Path, destination: Path) -> str:
    source_literal = _character_code_expression(source)
    destination_literal = _character_code_expression(destination)
    return f"""#target illustrator
(function () {{
...
def run_illustrator_modern_roundtrip_test(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="py-ai-modern-roundtrip-") as directory:
        ...
        try:
            completed = _execute_javascript(
                _build_modern_roundtrip_javascript(input_copy, resaved),
                root,
                timeout=timeout,
                application_name=application_name,
            )
        ...
        visual_change_within_tolerance = (
            ...
            and all(
                ...
                for before_page, after_page, diff_page in zip(
                    difference.before.pages,
                    difference.after.pages,
                    difference.pages,
                    strict=True,
                )
            )
        ...
        checks = {
            "resave_created_pdf_compatible_ai": (
                inspect_file(resaved).format is FileFormat.PDF_COMPATIBLE_AI
            ),
            ...
            "pdf_and_private_timestamps_match": (
                display.private_data_freshness != "timestamp_mismatch"
            ),
